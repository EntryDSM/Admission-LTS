...
const GradePreview = ({ gradeCurrent, selectGradeElement, writeGradeElement }: IGradePreview) => {
  ...
  useEffect(() => {
    setScore({
      gradeScore: getSelectGradeScore(gradeCurrent, isCommon, selectGradeElement, isGraduate),
      attendenceScore:
  ...
  return (
    <CurrentGrades>
      <Text size="body2" color="black" style={{ fontWeight: 700 }}>
        총: {score.gradeScore + score.attendenceScore + score.volunteerScore + score.extraScore}/{score.maxScore}
      </Text>
